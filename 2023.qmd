...
# see: lab_sample_id = 1233640028 has collect date in spring and receive date in summer. fix me 
...
# PRIORITY TASK, WORKING HERE - sent email to c whisman 5/14/2024


# WORKING HERE TO DIAGNOSE JOIN ISSUE(S), WHY 3 OBS PER PARAMETER PER SITE/DATE...?

# something with SGS read in is getting mis-assigned. compare 1-parameter/site read results here (3x) vs in intial read in (2x; in obs and one dup ...) where is the extra one coming from? compare results here vs @ read in to diagnose. Goal: 5:30 AM.

# issue might not be just join - why the odd number of observations per site/date

# tangential possible issue -- are there multiple times associated with sampling events? find out

# result_analytical_method_id missing for many metals observations

z <- dat %>%
  filter(sample_type == "PS",
         characteristic_name == "Iron") %>%
  select(activity_id,monitoring_location_id,activity_start_date,activity_type,characteristic_name,result_value,result_unit,result_sample_fraction) 
  distinct(activity_start_time)
  
  # WORKING HERE new mystery: some field blanks have no result_analytical_method_id
...
z <- inner_join(dat_metals_tot,dat_metals_diss,
                by = c("characteristic_name","activity_start_date","monitoring_location_id","field_sample_type","activity_type")) %>%
  distinct() %>%
  arrange(monitoring_location_id,characteristic_name)


# nonsensical replicates apparent after join
# no FB results present

# issue: lots of spring results where tot < diss. incorrectly assigned method likely. also dup obs ?!? (many to many)

# check pdfs to verify not crazy

...
dat %<>%
    mutate(rec_time = strptime(rec_time, format = "%H:%M:%S","GMT"),
           activity_start_time = strptime(activity_start_time, format = "%H:%M:%S","GMT"))

# in progress: trying to convert characters to time formats
...
z <- dat %>%
  mutate(hold_time = as_hms(activity_start_time - rec_time)) %>%
  filter(sample_type == "PS")

# working here 3/4/2024

# getting all nonsense values for hold time

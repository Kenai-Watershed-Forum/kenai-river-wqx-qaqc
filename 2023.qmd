...
# subset dataframes to set of desired final colnames based on 2021 example
...
sgs23 %<>% select(one_of(col_names)) 
fc23 %<>% select(one_of(col_names)) 
tss23 %<>% select(one_of(col_names))


# we need to synchornize column types for all names in the above 3 dataframes

# WORKING HERE 2/19/2024

# suggestion: write function that assigns class by column name; apply function to each data frame individually
# if successful, save function to separate R script and use source()

source("other/functions/transform_columns.R")



# error
sgs23 <- set_column_classes(df = sgs23,class_list)
...
# Set the classes
df <- set_column_classes(df, class_list)
...
# error
z <- set_column_classes(sgs23, class_list)

# possible disgnosis strategy: try reducing sgs23 to 2 columns as a time?

z1 <- sgs23 %>% select(monitoring_location_id)


z2 <- set_column_classes(z1,class_list)

# WORKING HERE 2/23/2024

# this approach seems to work for the first column at least. The issue must be cuased by a particular column (unknown which one). Continue adding across one at a time until identified.




# assign column classes example

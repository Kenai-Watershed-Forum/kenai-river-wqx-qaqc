...
sgs23 %<>%
  ...
  mutate(activity_type = case_when(
    ...
    TRUE ~ activity_type)) %>%
  # designate activity type abbreviations
...
# *N" is not an option present in CDX; all other choices are. We will modify "*N" to simply "*" for simplification, for it's one instance as an MSD sample (matrix spike duplicate; non-field). Rationale: the "N" part of this qualifier is not mandatory information.
sgs23 %<>% 
...
# WORKING HERE 2/13/2024

# Result Detection Condition	
# categorical choice based on result qualifier. Import matching table.

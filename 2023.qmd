...
z <- dat %>%
  ...
  # correct typo showing lab QA/QC analyses occurring prior to summer field sampling on 2023-07-18
  mutate(activity_start_date = case_when(
    
  ))

# RUN_DATE_TIME data for some summer sgs data is missing here, but appears to be present in EDD. diagnosing 2/28/2024

# missing activity start date column for fc23 ... not sure why!!
z <- dat %>%
  filter(characteristic_name == "Fecal Coliform")

# WORKING HERE 2/28/24
...
# transform column classes as necessary for QA/QC analyses
...
Yes, with small exceptions. One site in spring 2023 could not be accessed for sampling due to low water conditions (Kenai River main stem, RM 10.1).

...
# These sites include "No Name Creek" , "Skilak Lake Outflow," and "Jim's Landing
...
lat_long <- read_csv("other/input/2023/misc/lat_long.csv") %>%
  transform(monitoring_location_id = as.character(monitoring_location_id))
z <- sgs23 %>% left_join(sgs23,lat_long, relationship = "many-to-one", by = "monitoring_location_id")


# working to address error here 1/31/24
# join results in many-to-many, unclear why!
```
...
# FORMULAIC final steps for combined dataframe
# (use cdx domain values)

# activity_id (concatenate, use abbrv)
# Result Analytical Method ID (matching tto cdx)
# Result Analytical Method Context (matching to cdx)
# Result Qualifier (matching, verify symbols; SGS terms vs CDX terms)
# Result Sample Fraction (matching)
# Method Speciation (matching)
# Sample Collection Equipment Name (matching)
# Result Sample Fraction (matching)
# Result Detection Condition (calculated)
# Result (see "note on non-detect values")
# Result Status ID (matching from manual table..? check domains for various options)
# Sample Container Type (matching)
# Sample Container Color (matching)
# Chemical Preservative Used (matching)



# STATIC final steps for combined dataframe
# all other columns

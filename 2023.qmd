...
# read in 2021 column names
col_names <- read.csv("other/output/example_output/results_activities_2021.csv") %>%
  clean_names() %>%
  colnames() 

z <- read.csv("other/output/example_output/results_activities_2021.csv") %>%
  clean_names()

write.csv(data.frame(col_names), "other/input/2023/misc/col_names.csv", row.names = F)
...
# 2/16/2024 - working here -- how to assign column types in bulk based on name matching??
# https://www.google.com/search?q=dplyr+Use+%60spec%28%29%60+for+the+full+column+specifications.%0D%0A%0D%0A&sca_esv=5ea4c8dbbd821d13&ei=zRXQZbKtCoqD0PEPrP626AY&ved=0ahUKEwjyx6eFprGEAxWKATQIHSy_DW0Q4dUDCBA&uact=5&oq=dplyr+Use+%60spec%28%29%60+for+the+full+column+specifications.%0D%0A%0D%0A&gs_lp=Egxnd3Mtd2l6LXNlcnAiOGRwbHlyIFVzZSBgc3BlYygpYCBmb3IgdGhlIGZ1bGwgY29sdW1uIHNwZWNpZmljYXRpb25zLgoKSABQAFgAcAB4AZABAJgBAKABAKoBALgBA8gBAPgBAfgBAg&sclient=gws-wiz-serp


# retain desired columns from each dataframe
sgs23 %<>% select(one_of(col_names)) %>% type_convert()
fc23 %<>% select(one_of(col_names)) %>% type_convert()
tss23 %<>% select(one_of(col_names))

z <- bind_rows(sgs23,fc23)

...
# WORKING HERE 5/10/2024 -- NEED TO DISTINGUISH SUMMER VS SPRING TOTAL METALS RESULTS - DONE - USE "ANALYSIS GROUP"


# method SW846 6010D pertain to metal digestion and does not have reportable values. It is is included in the SGS records here as project sample ("PS") at select sites for unclear reasons.

z <- sgs23 %>% filter(sample_type == "PS",
                      collect_date == "2023-07-18")
...
sgs23 %<>%
  ...
  mutate(result_sample_fraction = case_when(
    ...
    # WORKING HERE - need to designate spring 2023 as 200.8 = dissolved, and summer 2023 as 200.8 = dissolved WHEN [200.8 + analysis_group = "Dissolved Metals by ICP/MS"]
    
    
    grepl("EP200.8|EP200.8 ", analytical_method)
            
    
    
    
    
    analytical_method == "EP200.8|EP200.8 " &                           # total metals as used in summer 2023 
      analysis_group == "Metals by ICP/MS" ~ "Unfiltered",
    analytical_method == "EP200.8|EP200.8 " &                                    # dissolved metals as used in summer 2023
...
z <- sgs23 %>% filter(sample_type == "PS") %>%
  select(sample_id, collect_date,analytical_method, sample_type,result_sample_fraction)
...
Yes, with the exceptions discussed in question #1. The SGS project manager describes that no change is anticipated for how to interpret results.
...
Numerous corrections to site names and dates were necessary to prepare data into final form. Original documents (Electronic Data Deliverables) were not modified, and corrections are applied throughout the data read-in process using R coding script in this document.
...
# PRIORITY TASK, WORKING HERE - sent email to c whisman 5/7/2024
...
# note that analytical method ID is not included in the above dataframe. This is because in summer 2023, 200.8 was used for both total AND dissolved metals
...
# results here are nonsensical; missing almost all spring 2023 results
...
(Note: metals parameters not specified in the QAPP are excluded from these analyses. In 2021, SGS laboratories provided dissolved metals results for parameters beyond what was originally requested.) \>\>\>\>\>\>\> 316dfab7a4e0e74b4522c2c6c0aa1010af44ec8d

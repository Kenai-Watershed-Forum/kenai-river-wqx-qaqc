...
# method SW846 6010D pertain to metal digestion and does not have reportable values. It is is included in the SGS records here as project sample ("PS") at select sites for unclear reasons. We will remove them.
...
z <- sgs23 %<>%
  ...
  mutate(result_sample_fraction = case_when(
    ...
    # by analysis_group
    analysis_group = "Metals by ICP/MS|Metals by ICP/MS (Provisional for Be, Cu 06052023)|Metals", analysis_group ~ "Unfiltered"))


# WORKING HERE TO ASSIGN WHICH FRACTION AS DISSOLVED AND WHICH TOTAL !!!!

    
    # this line is not working to asssign
    
    grepl("Diss.|Dissolved", analysis_group) ~ "Filtered, lab",
    TRUE ~ "")) %>%
...
# PRIORITY TASK, WORKING HERE - sent email to c whisman 5/13/2024
...
# NOTE that analytical method ID is not included in the above dataframe. This is because in summer 2023, 200.8 was used for both total AND dissolved metals. May choose to modify in future emplates
...
# issue: lots of spring results where tot < diss. incorrectly assigned method likely. also dup obs ?!?
```

...
# calculate limitations on what can and can't be used for RPD calculations

# calculate if samples are above LOQ and/or 2xLOQ
...
# arrange values side by side by joining tables
dup_vals1 <- dup_values %>%
  filter(grepl("DUP",activity_id)) %>%
  rename(dup_result_value = result_value,
         dup_result_qualifier = result_qualifier,
         loq_pass1 = loq_pass,
...
dup_diffs <- left_join(dup_vals1,dup_vals2) %>%
  # determine if pair of values is eligible to calculate RPD
  mutate(rpd_elgible = case_when(

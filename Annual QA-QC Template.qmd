...
```{r, Annual WQX formatting for SGS, echo = F, message = F}
...
# assign project year and sampling dates, modify to annual dates
activity_year <- as.character("####")
spring_sample_date <-  ymd("1999-09-19")
summer_sample_date <- ymd("1999-09-19")

```

## Introduction

\[10/23/2024: Working to make this script in to a year-less template\]
...
The QAPP for this project describes data management details and responsible parties for each step of the data pipeline from observation to repository. The `r activity_year` data preparation and review process is published here.

### Year `r activity_year` Water Quality Data

In this chapter we will collate `r activity_year` laboratory data from several sources into a single spreadsheet document with a consistent format. The desired end format is a spreadsheet template provided by the EPA Water Quality Exchange. These template files are available to download from the EPA at <https://www.epa.gov/waterdata/water-quality-exchange-web-template-files>.
...
Data that has been uploaded to the EPA WQX is evaluated biannually by the Alaska Department of Environmental Conservation (ADEC) in their [Integrated Water Quality Monitoring and Assessment Report](https://dec.alaska.gov/water/water-quality/integrated-report/)[^annual-qa-qc-template-1]. The integrated report evaluates available water quality data from the previous five years against Alaska water quality standards and regulations [@adec2020].

[^annual-qa-qc-template-1]: https://dec.alaska.gov/water/water-quality/integrated-report/

#### `r activity_year` Water Quality Data Formatting



The code scripts in this document assemble water quality data from the two analytical laboratories that partnered with Kenai Watershed Forum for this project in `r activity_year`:

-   LABORATORY NAME A (CITY, STATE)

-   LABORATORY NAME B (CITY, STATE)

<br>

------------------------------------------------------------------------

##### `r activity_year` Metals/Nutrients Lab Results (SGS Labs)

```{r, echo = F, message = F}
library("xfun")

# specify directories for spring/summer metals & nutrients results; EDD and PDF
dir_edd_spring_results <- paste0("other/input/",activity_year)
dir_edd_summer_results <- paste0("other/input/",activity_year)

xfun::embed_file(dir_spring_edd_results, text = paste("Download Original Spring",activity_year,"Metals/Nutrients Lab Results - Electronic Data Delivery file"))

xfun::embed_file(dir_summer_edd_results, text = paste("Download Original Summer",activity_year,"Metals/Nutrients Lab Results - Electronic Data Delivery file"))

```

```{r, echo = F}
# specify directories for spring/summer metals & nutrients results; PDF
dir_pdf_spring_results <- paste0("other/input/",activity_year)
dir_pdf_summer_results <- paste0("other/input/",activity_year)

xfun::embed_file(dir_spring_pdf_results, text = paste("Download Original Spring",activity_year,"Metals/Nutrients Lab Results - PDF file"))

xfun::embed_file(dir_summer_pdf_results, text = paste("Download Original Summer",activity_year,"Metals/Nutrients Lab Results - PDF file"))
...
# working to generalize steps below here 10/24/2024
```
...
############################ Part A.1: Metals/Nutrients Data Read In #############################
...
60 of 1280 total observations are flagged, resulting in a Completeness Measure A value of 95.3%.
...
As noted in question #2, one site (RM 10.1) was not visited in spring 2023, and in addition one fecal coliform sample was not collected in spring 2023. These planned sample constitute a total of
...
#summarise
chr_num <- dat %>%
  filter(characteristic_name != "" | !is.na(characteristic_name)) %>%

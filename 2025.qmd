...
spring_data_dir <- file.path("other/input/2025/spring_2025_wqx_data")
summer_data_dir <- file.path("other/input/2025/summer_2025_wqx_data")
output_qaqc_dir <- file.path("other/output", year, "qa_qc_output")
...
dir.create("other/output/lab_qaqc_data",                    recursive = TRUE, showWarnings = FALSE)
```
...
# ── Derive sample_condition from sample_id string ──────────────────────────────
sgs_raw <- sgs_raw |>
  mutate(
    sample_condition = case_when(
      grepl("DUP|Dup",             sample_id, ignore.case = FALSE) ~ "Field Duplicate",
      grepl("Field Blank|Field B", sample_id, ignore.case = TRUE)  ~ "Field Blank",
      grepl("Trip Blank",          sample_id, ignore.case = TRUE)  ~ "Trip Blank",
      grepl("Method Blank",        sample_id, ignore.case = TRUE)  ~ "Method Blank",
      TRUE ~ NA_character_
    )
  )
...
# ── Join site names matching table (sample_id -> monitoring_location_id) ────────
sgs_site_names <- read_excel(
  file.path(cfg$templates_dir, "sgs_site_names_matching_table.xlsx")
) |>
  clean_names() |>
  select(sgs_sample_id, monitoring_location_id) |>
...
ysi_path <- file.path("other/input/2025",
...
source("other/misc/qaqc_repo_transition/functions/format_wqx.R")
```
...
write.csv(rpd_check_dat,
          "other/output/field_qa_qc_data/rpd_check_dat.csv",
          row.names = FALSE)
...
source("other/misc/qaqc_repo_transition/functions/apply_qaqc_flags.R")
```
...
write.csv(cmb_by_method,
          "other/output/field_qa_qc_data/completeness_measures/cmb_parameter.csv")
...
source("other/misc/qaqc_repo_transition/functions/generate_cdx_export.R")
```
